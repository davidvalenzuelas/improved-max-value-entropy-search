...
def main():
    ...
    problem = synthetic_problem.f_noisy
    ...
    for bo_iteration in range(BO_iters):
        ...
        if model is not None:
            model = fit_model(x_observations, y_values_obs.detach(), model.state_dict(), likelihood_exp = "GAUSSIAN")
        else:
            model = fit_model(x_observations, y_values_obs.detach(), likelihood_exp = "GAUSSIAN")
