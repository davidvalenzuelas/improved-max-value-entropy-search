#!/usr/bin/env python3
# coding: utf-8
"""Test JES acquisition object on a 1D problem."""
...
from my_acquisition_jes import MyAcquisition
...
def plot_predictive(ax, x_grid: torch.Tensor, x_train: torch.Tensor, y_train: torch.Tensor,
    ...
    x_star: float, x_star_t: torch.Tensor, y_star_t_col: torch.Tensor,
    ...
    ax.plot(x_star_t.squeeze(-1).cpu().numpy(), y_star_t_col.reshape(-1).cpu().numpy(),
        marker="o", linestyle="None", color="tab:green", markersize=7,
        label="Sampled optimum")
...
def main():
    ...
    if exact_cond is None:
        ...
        format_diff_stats(
            ...
            "JES Gaussian truncation", mean_trunc_y, var_trunc_y)
        format_diff_stats(
            "Exact JES conditional approx", exact_cond.mean_y, exact_cond.var_y,
            "JES acquisition object model", mean_obj_y, var_obj_y)
    ...
    if exact_cond is not None:
        ...
        summarize_acquisition_curve("   Exact JES conditional", x_grid, acq_exact)
    else:
        acq_exact = None
    summarize_acquisition_curve("   JES Gaussian truncation", x_grid, acq_trunc)
    ...
    train_Y_aug = torch.cat([
        y_train,
        y_star_t_col.reshape(-1).to(dtype=y_train.dtype, device=y_train.device),
    ], dim=0)
    ...
    ax.plot(x_np, normalize_acquisition(acq_from_object).reshape(-1).cpu().numpy(),
        linewidth=2.4, label="JES_Object_Acq_Norm")
    ax.axvline(float(x_star), color="lightgreen", linestyle=":", label="x*")
    ax.legend(fontsize=7, loc="best")
