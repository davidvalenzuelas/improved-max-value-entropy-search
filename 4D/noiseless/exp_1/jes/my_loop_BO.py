...
# Defaults for BO optimize_acqf
DEFAULT_NUM_RESTARTS = 20
DEFAULT_RAW_SAMPLES = 512
# Defaults for sparse GP training
DEFAULT_M = 64
...
def get_maximum_problem(num_dims, problem):
    ...
    result = sp.optimize.fmin_l_bfgs_b(f_np, x0.cpu().numpy(), None,
        bounds=[(0.0, 1.0)] * num_dims, approx_grad=True,)
    
    x_opt = torch.from_numpy(result[0]).double()
    y_opt = problem(x_opt.view(1, num_dims)).view(-1)[0].double()
    return x_opt, y_opt

# This function optimizes the posterior mean of the model
...
# This function fits a VFE sparse GP to the data, with possible conditioning to y* (for JES)
def fit_model(train_X, train_Y, state_dict=None, *, M=DEFAULT_M, training_iter=DEFAULT_TRAINING_ITER,
    lr=DEFAULT_LR, noise_eps=DEFAULT_NOISE_EPS, y_star=None, Xc = None, num_constraint_points=DEFAULT_NUM_CONSTRAINT_POINTS,
    tau=DEFAULT_TAU, mc_samples=DEFAULT_MC_SAMPLES, constraint_weight=DEFAULT_CONSTRAINT_WEIGHT,):
    
    model, likelihood = fit_model_vfe_sparse(train_X=train_X, train_Y=train_Y, state_dict=state_dict,
        M=M, training_iter=training_iter, lr=lr, noise_eps=noise_eps, verbose=False, y_star=y_star,
        Xc=Xc, num_constraint_points=num_constraint_points, tau=tau, mc_samples=mc_samples,
        constraint_weight=constraint_weight,)
    model.likelihood = likelihood
    return model

# This functions computes optimal samples for JES by sampling from the posterior
def get_optimal_samples_grid_mc(model, bounds: torch.Tensor, num_optima: int, num_grid: int = 4096):
    ...
    with torch.no_grad():
        ...
        samples = post.rsample(base_samples=base_samples)
...
def main():
    ...
    np.savetxt(
        config["file_results"] + "/my_x_optimum_problem.txt",
        x_star.detach().cpu().numpy().reshape((1, num_dims)),
    )
    np.savetxt(
        config["file_results"] + "/my_y_optimum_problem.txt",
        np.array([float(y_star.detach().cpu().numpy())]),
    )
    
    # Initial observations
    x_observations = torch.rand(num_initial_obs, num_dims, dtype=torch.double)
    y_values_obs = problem(x_observations).double().view(-1, 1)
    
    # Resumes if exists
    if os.path.exists("my_points_evaluated.txt") and os.path.exists("my_y_values_evaluated.txt"):
        x_observations = torch.from_numpy(np.loadtxt("my_points_evaluated.txt", ndmin=2)).double()
        y_loaded = torch.from_numpy(np.loadtxt("my_y_values_evaluated.txt", ndmin=2)).double()
        y_values_obs = y_loaded.view(-1, 1)
        
        # Adjusts remaining iterations
    ...
    # Fixed constraint points Xc for conditioning to y* (JES)
    Xc = draw_sobol_samples(bounds=bounds.double(), n=num_constraint_points, q=1,).squeeze(1).double()
    ...
    for bo_iteration in range(BO_iters):
        ...
        if model is not None:
            ...
            model = fit_model(x_observations, y_values_obs.detach(), state_dict=prev_state, M=M,
                training_iter=training_iter, lr=lr, noise_eps=noise_eps, y_star=y_star, Xc=Xc,
                num_constraint_points=num_constraint_points, tau=tau, mc_samples=mc_samples,
                constraint_weight=constraint_weight,)
        else:
            model = fit_model(x_observations, y_values_obs.detach(), state_dict=None, M=M,
                training_iter=training_iter, lr=lr, noise_eps=noise_eps, y_star=y_star, Xc=Xc,
                num_constraint_points=num_constraint_points, tau=tau, mc_samples=mc_samples,
                constraint_weight=constraint_weight,)
        ...
        # Chooses acquisition and optimizes it
        print("Optimizing acquisition ...")
        if acquisition_name == "JES":
            # Builds botorch model wrapper with conditioning to y* (for JES)
            model_botorch = as_botorch_model(model, y_star=y_star, Xc=Xc,
                num_constraint_points=num_constraint_points,tau=tau,
                mc_samples=mc_samples, constraint_weight=constraint_weight,)
            
            acq = qJointEntropySearch(model=model_botorch, optimal_inputs=optimal_inputs.double(),
                optimal_outputs=optimal_outputs.double(), estimation_type="LB", condition_noiseless=True,)
            
            candidate, acq_value = optimize_acqf(acq_function=acq, bounds=bounds, q=1, num_restarts=num_restarts_opt,
        ...
        # 3 recommendations to evaluate at the end of the iteration
        ...
        y_new = problem(candidate).double().view(-1, 1)
        ...
        np.savetxt("my_points_evaluated.txt", x_observations.detach().cpu().numpy())
        np.savetxt("my_y_values_evaluated.txt", y_values_obs.detach().cpu().numpy())
